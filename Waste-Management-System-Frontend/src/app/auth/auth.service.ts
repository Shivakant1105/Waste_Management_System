...
export class AuthService {
  ...
  userSignUp(data: any) {
    this.http
      .post(this.backendLink + "/api/customer/signup", data)
      .subscribe((result) => {
        console.log(result);
  ...
  userLogin(data: { email: string; password: string }) {
    ...
    if (
      data.email.split("@")[1] === "superadmin.com" ||
      data.email.split("@")[1] === "admin.com"
    ) {
      this.http
        .post<{
          message: string;
      ...
      this.http
        .post<{
          ...
          expiresIn: number;
          email: string;
          driverRegionCode: string;
        }>(this.backendLink + "/api/driver/driverLogin", data)
        .subscribe((response) => {
          console.log(response);
          const token = response.token;
          this.token = token;
